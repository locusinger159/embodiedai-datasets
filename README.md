...
│   │   ├── navbar.html            #   导航栏（含 GitHub CTA）
│   │   └── footer.html            #   4 列页脚
│   └── pages/                     # 页面模板
│       ├── index.html             #   首页（Hero + 统计 + 特性 + 合作机构）
...
├── docs/
│   ├── data/
│   │   ├── datasets.json          # 数据集数据（55 条，唯一数据源）
│   │   └── standards.json         # 数据标准（19 条）
│   └── public/                    # 静态资源（logo、favicon、CNAME）
├── build.cjs                      # 构建脚本（零依赖 Node.js）
...
1. `node build.cjs` — 读取 JSON，注入模板，生成 55 个详情页 + 4 个列表页
